(() => {
  ...
  const saveCards = (cards) => {
    ...
    localStorage.setItem('erpBonusCustomers', JSON.stringify(cards));
  };
  ...
  const fetchCards = async () => {
    try {
      ...
      if (!response.ok || data.status !== 'success') {
        throw new Error(data.message || 'Müştəri kartları yüklənmədi.');
      }
  ...
  const render = async () => {
    ...
    list.querySelectorAll('[data-customer-edit]').forEach((button) => {
      button.addEventListener('click', () => {
        openModal(cards.find((card) => String(card.id) === String(button.dataset.customerEdit)));
      });
    });
  ...
  const resetForm = () => {
    form.reset();
    field('id').value = '';
    field('bonus-balance').value = '0';
    document.querySelector('#customer-card-title').textContent = 'Yeni müştəri kartı';
  };
  ...
  const openModal = (card = null) => {
    ...
    if (card) {
      ...
      const values = {
        ...
        'bonus-balance': card.bonusBalance,
  ...
  field('card-number').addEventListener('input', (event) => {
    event.target.value = event.target.value.replace(/\D/g, '').slice(0, 16);
  });

  field('bonus-balance').addEventListener('input', (event) => {
    event.target.value = event.target.value.replace(/\D/g, '');
  });

  field('limit').addEventListener('input', (event) => {
    event.target.value = event.target.value.replace(/\D/g, '');
  });
  ...
  form.addEventListener('submit', async (event) => {
    ...
    try {
      ...
      const savedCard = normalizeCard(data.customer || { ...payload, id: data.customer?.id || id || Date.now() });
      const cards = getCards();
      const index = cards.findIndex((item) => String(item.id) === String(savedCard.id));
      const nextCards = [...cards];
      if (index >= 0) nextCards[index] = savedCard; else nextCards.push(savedCard);
      saveCards(nextCards);
      ...
      if (autoCreate?.type === 'customer') {
        window.__erpTriggerAfterCreate?.(savedCard);
      } else {
        const fallbackSelector = window.__erpSelectionTarget?.selector || '#sales-customer';
        const fallbackTarget = document.querySelector(fallbackSelector);
        if (fallbackTarget && savedCard?.name) fallbackTarget.value = savedCard.name;
